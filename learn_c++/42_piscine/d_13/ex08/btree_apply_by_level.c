...
/*   Updated: 2016/07/29 15:22:17 by sqian            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		level_at(t_rb_node *root, int curlevel, int level,
		void (*applyf)(void *item, int current_level, int is_first))
{
	...
	if (curlevel == level)
	{
		applyf(root->data, curlevel, !g_flag);
		if (!g_flag)
			g_flag = 1;
		return (1);
	}
...
void	btree_apply_by_level(t_rb_node *root, void (*applyf)(void *item,
