...
class segTree{
    ...
    vector<node> tree;
    int sz;
    vector<int> lazyPropagation;// different
    segTree(vector<int>& array){
        sz = (int)ceil(log(array.size())/log(2));
        sz = 2*pow(2,sz)-1;
        tree = vector<node>(sz, node(0,0));
        lazyPropagation = vector<int>(sz, 0);// different
        construct((int)array.size());
        construct_r(array, 0);
    }
    void lazyProp(int index){
        if (lazyPropagation[index] != 0) {
            tree[index].val += lazyPropagation[index];
            if (tree[index].lo < tree[index].hi) {
                lazyPropagation[2*index+1] += lazyPropagation[index];
                lazyPropagation[2*index+2] += lazyPropagation[index];
            }
            lazyPropagation[index] = 0;
        }
    }
    int setValue_r(int lo, int hi, int index, int change){
        lazyProp(index); // different
        if (tree[index].lo > hi || tree[index].hi < lo) return tree[index].val;
        if (tree[index].lo == tree[index].hi) return tree[index].val+=change;
        if (tree[index].lo >= lo && tree[index].hi <= hi) {
            tree[index].val += change;
            lazyPropagation[2*index+1] += change;
            lazyPropagation[2*index+2] += change;
