...
class IndexPQ {
  public:
    IndexPQ();
    void insert(T key, double nb);
    void modify(T key, double n_nb);
    void erase(T key);
    pair<T, double> minPQ();
...
template <typename T>
void IndexPQ<T>::insert(T key, double nb){
  int cur_index = (int)pq.size();
  index_map[key] = cur_index;
  pq.push_back(pair<T, double>(key, nb));
...
template <typename T>
pair<T, double> IndexPQ<T>::minPQ(){
  return pq[0];
}

template <typename T>
double IndexPQ<T>::operator[](T key){
...
template <typename T>
void IndexPQ<T>::modify(T key, double n_nb){
